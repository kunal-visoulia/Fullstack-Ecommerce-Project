...
26. BE Config Code refactoring: Fix deprecated method and Configure CORS mapping for spring Data REST(had already done ==> adding CorsRegistry cors param to configureRepositoryRestConfiguration method of MyDataRestConfig.java), Configure CORS mapping for @RestController in MyAppConfig.java that implements WebMvcConfigurer(this config is seperate from Spring Data REST config in MyDataRestConfig.java), Disable HTTP Patch method(even though we configured some API's to be read-only by disabling PUT,POST and DELETE; someone can still patch data to those end pts) and Modify Spring Data REST Detection Strategy.